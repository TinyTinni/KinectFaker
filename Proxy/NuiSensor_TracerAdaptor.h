...
class INuiSensor_TracerAdaptor : public INuiSensor
{
    std::shared_ptr<spdlog::logger> m_log;
    std::unique_ptr<INuiSensor> m_sensor;
public:

    ~INuiSensor_TracerAdaptor();

    //INuiSensor_Faker(kif::Scene s, _bstr_t connectionId, int index);
    INuiSensor_TracerAdaptor(std::shared_ptr<spdlog::logger> logger, std::unique_ptr<INuiSensor> sensor);


    // INuiSensor overloaded functions
    virtual ULONG STDMETHODCALLTYPE Release();
    virtual ULONG STDMETHODCALLTYPE AddRef();
    virtual HRESULT STDMETHODCALLTYPE QueryInterface(const IID& i, void** v);
